...
###  4.0 CONCLUSION
1. 50% of Users are not getting up to 7 hours (420 minutes) of sleep. Bellabeat Time  product can be used to improve it with the  use of  notifications and targets.
2. 54% of the Users are taking less steps than the average. Bellabeat Tracker product can be used to improve users steps by setting targets for the users.
3. 42% of Users are performing badly with less steps, active minutes and sleep than the average. Bellabeat lifestyle product can be used to rate users on how they are fairing in different metrics and set target for each of the metrics that users can meet to ensure a healthy lifestyle.
4. More users are having more sedentary minutes than the average. Bellabeat Tracker product can used to notify users if their sedentary minutes is above the average or a target.
5. Users are having more active week days than weekends. Bellabeat send notifications to users to remember to take it's hydration product on very active days